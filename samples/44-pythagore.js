...
$(function() {
    ...
    function go() {
		if (step == 0) {
			move_up();
			$('#go').removeClass("btn-success").addClass("btn-info").html('Continue ...')
			step = 1;
		} else if (step == 1) {
			turn();
			$('#go').removeClass("btn-info").addClass('btn-danger').html('Reset')
			step = 2;
		} else {
			reset();
			$('#go').removeClass("btn-danger").addClass('btn-success').html('Go !')
			step = 0;
		}
    }
